...
namespace MS_EntWatch.Modules
{
    class UHudItem
    {
        ...
        public void SetButtonSeparator(ICustomHudLayout eHud, IPlayerController HudPlayer)
        {
            if (bNull)
            {
            ...
            if (!bSeparator)
            {
                eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_Button, "separator", HudPanelClassStatus.ForceEnable);
        ...
        public void SetButtonNull(ICustomHudLayout eHud, IPlayerController HudPlayer)
        {
            if (bSeparator)
            {
                eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_Button, "separator", HudPanelClassStatus.ForceDisable);
                bSeparator = false;
            }
            if (!bNull)
            {
        ...
        public void ChangeValue(ICustomHudLayout eHud, IPlayerController HudPlayer, string _ItemNameText, string _ItemColorClass, byte _TextSize, string _CooldownText, string _CooldownColorClass, string _NicknameText, string _NicknameColorClass, string _SteamIDText, string _SteamIDColorClass, byte _ProgressValue)
        {
            ...
            if (iTextSize != _TextSize)
            {
                void UpdateSizeClasses(int iTextSizeValue, HudPanelClassStatus status)
                {
                    ...
                    eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_ItemName, sSizeAll, status);
                    eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_Cooldown, sSizeAll, status);
                    eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_Nickname, sSizeAll, status);
                    eHud.SetClassOverrideForPlayer(HudPlayer, __Cache_Steam, sSizeSteamID, status);
